...
- `npm test` / `npm run test:smoke` — run the full Playwright suite (currently equivalent aliases; both run every spec). 24 tests: 21 in `tests/smoke.spec.mjs` plus 3 runtime-behavior tests in `tests/runtime.spec.mjs`.
