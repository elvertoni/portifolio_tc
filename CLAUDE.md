...
Framework-free static portfolio site (Toni Coimbra). Pure HTML5/CSS3/ES6+, no build step, no bundler, no runtime dependency. `index.html` loads `assets/css/style.css` and `assets/js/main.js` (defer), both with a `?v=YYYYMMDD` cache-busting query — bump it on both tags plus the favicon link when shipping a visual change. **`design-system/design-system.html` carries the same query on every asset it references** (stylesheet, `gallery.js`, and each `logo.svg`/`favicon.svg`), and bumping means bumping both pages. This is not belt-and-braces: production sits behind Cloudflare, which overrides the `no-cache, must-revalidate` that `nginx.conf` sets for css/js/svg and serves them `public, max-age=604800`. An unversioned URL is a week-old file — that is how the catalog once shipped a new page against a stale stylesheet and the previous logo. A smoke test asserts no catalog asset is unversioned. Design system: "Volta Atelier" — brutalist-editorial, dark warm background (`#141414`), signal red accent (`#fb3732`).
...
- `npm test` / `npm run test:smoke` — run the full Playwright suite (currently equivalent aliases; both run every spec). 32 tests: 22 in `tests/smoke.spec.mjs`, 5 runtime-behavior tests in `tests/runtime.spec.mjs`, 5 gallery tests in `tests/gallery.spec.mjs`.
